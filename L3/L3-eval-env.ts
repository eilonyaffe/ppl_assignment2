...
const applyProcedure = (proc: Value, args: Value[]): Result<Value> =>
    ...
    isClassSExp(proc) ? applyClassSExp(proc, args) :
...
const applyClassSExp = (proc: ClassSExp, args: Value[]): Result<Value> =>{ 
    const fieldsForEnv = map((v:VarDecl)=>v.var, proc.fields);
    return makeOk(makeObjectEnv(proc, makeExtEnv(fieldsForEnv,args,proc.env)));//without eager substitution
}
    ...
    if(!isEmpty(args) && isSymbolSExp(args[0])){
        ...
        if(!isEmpty(correctMethod)){
            if(isProcExp(correctMethod[0].val)){
                return bind(applicativeEval(correctMethod[0].val, proc.env), (x:Value)=>applyProcedure(x, args.slice(1)));
            }
            else{
